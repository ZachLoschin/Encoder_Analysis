...
%% Plot the mean neural activity for each trial type
% Mean across trials (dim 3) for each trial type
mean_R1 = mean(probe1_trialdat(:, :, R1_Trials), 3); % timepoints x neurons
mean_R4 = mean(probe1_trialdat(:, :, R4_Trials), 3); % timepoints x neurons

mean_R1 = mean(mean_R1, 2);
mean_R4 = mean(mean_R4, 2);
figure()
plot(mean_R1)
hold on
plot(mean_R4)
...
% Mean across trials (dim 3) for each trial type
mean_R1 = mean(probe2_trialdat(:, :, R1_Trials), 3); % timepoints x neurons
mean_R4 = mean(probe2_trialdat(:, :, R4_Trials), 3); % timepoints x neurons
...
%% Plot firing rates vs trials to see if there is data loss
%% Plot firing rates vs trials to see if there is data loss
disp("in pro")
check1 = squeeze(mean(probe1_trialdat, 2)); % timepoints x trials

% --- Histogram ---
figure;
histogram(check1(:), 50, 'FaceColor', [0.2 0.5 0.8], 'EdgeColor', 'none', 'FaceAlpha', 0.75);
xline(mean(check1(:)), 'r-',  'Mean',  'LineWidth', 1.5, 'LabelVerticalAlignment', 'bottom');
xline(mean(check1(:)) + 2*std(check1(:)), 'r--', '+2σ', 'LineWidth', 1, 'LabelVerticalAlignment', 'bottom');
xline(mean(check1(:)) - 2*std(check1(:)), 'r--', '-2σ', 'LineWidth', 1, 'LabelVerticalAlignment', 'bottom');
xlabel('Mean voltage across channels');
ylabel('Count');
title('Probe 1 — trial mean distribution (disconnected trials will be outliers)');
box off;

% --- Detect bad trials ---
trial_means = mean(check1, 1);  % 1 x trials, mean over timepoints per trial
threshold   = mean(trial_means) - 2 * std(trial_means);
bad_trials  = find(trial_means < threshold);
good_trials = find(trial_means >= threshold);

fprintf('Removed %d bad trials: %s\n', length(bad_trials), num2str(bad_trials));

% --- Remove bad trials ---
% probe1_trialdat = probe1_trialdat(:, :, good_trials);
% check1          = check1(:, good_trials);

% --- Heatmap (timepoints x trials) ---
figure;
imagesc(check1');
colormap(parula);
colorbar;
clim_val = prctile(abs(check1(:)), 99);
clim([-clim_val clim_val]);
xlabel('Trials');
ylabel('Time (samples)');
title('Probe 1 — mean across channels per trial');
box off;
