...
probe1_R4_norm = zscore_pregc(probe1_R4, pre_gc_points, SR);  % -2 to 5s 100Hz
probe1_R1_norm = zscore_pregc(probe1_R1, pre_gc_points, SR);   % CHECK THIS, Pregcpoitns is 400 but only for -1->5s???
...
t_axis = linspace(-1, 5, num_timepoints1);  % adjust if probe1/probe2 differ in length
...
for pc = 1:n_pcs_to_plot
    ...
    sgtitle(sprintf('PC %d Comparison: Z-scored vs Mean-subtracted', pc), 'FontSize', 14);

    % --- Probe 1 ---
    % Z-scored
    ax1 = subplot(2, 2, 1);
    mean_zs_R1 = mean(Probe1_PCs_R1(:,:,pc), 2);
    mean_zs_R4 = mean(Probe1_PCs_R4(:,:,pc), 2);

    % single trials
    trial_idx = randperm(size(Probe1_PCs_R1, 2), min(n_single_trials, size(Probe1_PCs_R1,2)));
    for tr = trial_idx
        plot(t_axis, Probe1_PCs_R1(:,tr,pc), 'Color', [colors.R1, single_trial_alpha]); hold on;
    end
    trial_idx = randperm(size(Probe1_PCs_R4, 2), min(n_single_trials, size(Probe1_PCs_R4,2)));
    for tr = trial_idx
        plot(t_axis, Probe1_PCs_R4(:,tr,pc), 'Color', [colors.R4, single_trial_alpha]); hold on;
    end
    plot(t_axis, mean_zs_R1, 'Color', colors.R1, 'LineWidth', 2.5); 
    plot(t_axis, mean_zs_R4, 'Color', colors.R4, 'LineWidth', 2.5);
    xline(0, 'k--'); ylabel('PC score'); xlabel('Time from GC (s)');
    title('Probe 1 — Z-scored');
    legend('','','R1 mean','R4 mean', 'Location','northwest');

    % Mean-subtracted
    ax2 = subplot(2, 2, 2);
    mean_ms_R1 = mean(P1_ms_R1(:,:,pc), 2);
    mean_ms_R4 = mean(P1_ms_R4(:,:,pc), 2);

    trial_idx = randperm(size(P1_ms_R1, 2), min(n_single_trials, size(P1_ms_R1,2)));
    for tr = trial_idx
        plot(t_axis, P1_ms_R1(:,tr,pc), 'Color', [colors.R1, single_trial_alpha]); hold on;
    end
    trial_idx = randperm(size(P1_ms_R4, 2), min(n_single_trials, size(P1_ms_R4,2)));
    for tr = trial_idx
        plot(t_axis, P1_ms_R4(:,tr,pc), 'Color', [colors.R4, single_trial_alpha]); hold on;
    end
    plot(t_axis, mean_ms_R1, 'Color', colors.R1, 'LineWidth', 2.5);
    plot(t_axis, mean_ms_R4, 'Color', colors.R4, 'LineWidth', 2.5);
    xline(0, 'k--'); ylabel('PC score'); xlabel('Time from GC (s)');
    title('Probe 1 — Mean-subtracted');
    legend('','','R1 mean','R4 mean', 'Location','northwest');

    % --- Probe 2 ---
    ax3 = subplot(2, 2, 3);
    mean_zs_R1 = mean(Probe2_PCs_R1(:,:,pc), 2);
    mean_zs_R4 = mean(Probe2_PCs_R4(:,:,pc), 2);

    trial_idx = randperm(size(Probe2_PCs_R1, 2), min(n_single_trials, size(Probe2_PCs_R1,2)));
    for tr = trial_idx
        plot(t_axis, Probe2_PCs_R1(:,tr,pc), 'Color', [colors.R1, single_trial_alpha]); hold on;
    end
    trial_idx = randperm(size(Probe2_PCs_R4, 2), min(n_single_trials, size(Probe2_PCs_R4,2)));
    for tr = trial_idx
        plot(t_axis, Probe2_PCs_R4(:,tr,pc), 'Color', [colors.R4, single_trial_alpha]); hold on;
    end
    plot(t_axis, mean_zs_R1, 'Color', colors.R1, 'LineWidth', 2.5);
    plot(t_axis, mean_zs_R4, 'Color', colors.R4, 'LineWidth', 2.5);
    xline(0, 'k--'); ylabel('PC score'); xlabel('Time from GC (s)');
    title('Probe 2 — Z-scored');
    legend('','','R1 mean','R4 mean', 'Location','northwest');

    ax4 = subplot(2, 2, 4);
    mean_ms_R1 = mean(P2_ms_R1(:,:,pc), 2);
    mean_ms_R4 = mean(P2_ms_R4(:,:,pc), 2);

    trial_idx = randperm(size(P2_ms_R1, 2), min(n_single_trials, size(P2_ms_R1,2)));
    for tr = trial_idx
        plot(t_axis, P2_ms_R1(:,tr,pc), 'Color', [colors.R1, single_trial_alpha]); hold on;
    end
    trial_idx = randperm(size(P2_ms_R4, 2), min(n_single_trials, size(P2_ms_R4,2)));
    for tr = trial_idx
        plot(t_axis, P2_ms_R4(:,tr,pc), 'Color', [colors.R4, single_trial_alpha]); hold on;
    end
    plot(t_axis, mean_ms_R1, 'Color', colors.R1, 'LineWidth', 2.5);
    plot(t_axis, mean_ms_R4, 'Color', colors.R4, 'LineWidth', 2.5);
    xline(0, 'k--'); ylabel('PC score'); xlabel('Time from GC (s)');
    title('Probe 2 — Mean-subtracted');
    legend('','','R1 mean','R4 mean', 'Location','northwest');

    % Link y-axes within each probe for fair comparison
    % linkaxes([ax1 ax2], 'y');
    % linkaxes([ax3 ax4], 'y');
end
%% Plot PC trajectories
PC1_R1 = mean(Probe1_PCs_R1(:,:,1), 2);
PC1_R4 = mean(Probe1_PCs_R4(:,:,1), 2);
...
PC1_R1 = mean(Probe2_PCs_R1(:,:,1), 2);
PC1_R4 = mean(Probe2_PCs_R4(:,:,1), 2);
...
if hostname == "DESKTOP-5JJC0TM"
    outputFolder = fullfile(...
        'C:\Users\zlosc\Documents\GitHub\Encoder_Analysis\Preprocessed_Encoder\R14_2026_Test1d', ...
        [sessionName '_' sessionDate ]);
else
    outputFolder = fullfile( ...
        'C:\Research\Encoder_Modeling\Encoder_Analysis\Processed_Encoder\R14_2026_Test1d', ...
        [sessionName '_' sessionDate ]);
end
