...
summary_dir = 'C:\Research\Encoder_Modeling\Encoder_Analysis\Results_Window_R16\Summary_Figs_M1';
alt_base_dir = 'C:\Research\Encoder_Modeling\Encoder_Analysis\Processed_Encoder\R16';
subfolder = '';

%% Load metadata table
metadata = readtable('R16_Session_Metadata.xlsx');  % Your Excel saved as CSV

%% Filter the metadata for sessions to include
valid_sessions = metadata(metadata.To_include == 1 & strcmp(metadata.Location, 'M1'), :);
...
title('R16 Boxplot')
set(gca, 'XTickLabel', {})
saveas(gcf, fullfile(summary_dir, 'R16_Dt_Boxplot.png'))
saveas(gcf, fullfile(summary_dir, 'R16_Dt_Boxplot.fig'))
...
ylabel('Disengagement Time (s)')
xlabel(['Scaled Trial Count (/ ' num2str(scale_factor) ')'])
title('R16 Histogram (Scaled)')
saveas(gcf, fullfile(summary_dir, 'R16_Dt_Histogram_Scaled.png'))
saveas(gcf, fullfile(summary_dir, 'R16_Dt_Histogram_Scaled.fig'))
