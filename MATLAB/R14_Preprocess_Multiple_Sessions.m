...
%% Load excel file of sessions to process. Columns: Animal; Date

excel_file = 'C:\Research\Encoder_Modeling\Encoder_Analysis\MATLAB\R14_Animals.xlsx';

% Load the Excel data
session_table = readtable(excel_file, 'TextType', 'string'); % Ensures strings not char arrays

for i = 1:height(session_table)
    % Clear all variables except those needed to keep the loop running
    clearvars -except session_table i
    
    % Extract anm and date from current row
    anm = string(session_table.anm(i)); % Assuming there's an 'anm' column
    date_raw = session_table.date(i);   % Assuming there's a 'date' column
    
    fprintf('Processing animal %s, session %s\n', anm, date_raw);

    % Convert the date to a string with desired format
    if isdatetime(date_raw)
        date_str = datestr(date_raw, 'yyyy-mm-dd');  % e.g. '2024-11-10'
    elseif ischar(date_raw) || isstring(date_raw)
        % Try converting from slash to dash format
        date_str = regexprep(string(date_raw), '/', '-');
    else
    ...
    meta = loadZL(meta, datapth, anm, date_str);
    ...
    sessionName = char(meta.anm);            % make sure it's a character vector
    sessionDate = char(meta.date);           % should be '2024-11-10'
